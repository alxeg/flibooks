...
func ReadInpxFile(dataFile string, processor Processor) (err error) {
	...
	for i := 0; i < numProcesses; i++ {
		go processInp(files, results, done, processor)
	}

	waitAndProcessResults(done, results, processor)
...
func processInp(files <-chan *zip.File, results chan<- *models.Book, done chan<- struct{}, processor Processor) {
...
func waitAndProcessResults(done <-chan struct{}, results <-chan *models.Book, processor Processor) {
